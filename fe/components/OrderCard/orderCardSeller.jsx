...
const OrderCardSeller = React.memo(
  ({ order, onDelete, onAccept, onDelivering, onComplete }) => {
    ...
    const renderStatus = (status) => {
      switch (status) {
        ...
        case "preparing":
          return <Text style={styles.huyText}>Đang chuẩn bị</Text>;
        case "delivering":
          return <Text style={styles.huyText}>Đang vận chuyển</Text>;
        case "sold":
          return <Text style={styles.huyText}>Đã hoàn thành</Text>;
        default:
    ...
    const renderFooter = (status) => {
      switch (status) {
            ...
            <View style={[styles.namngang, { justifyContent: "flex-end" }]}>
              <TouchableOpacity
                style={[
                  ...
                  { backgroundColor: "white", borderColor: COLOR.mainColor },
                ]}
              >
                <Text style={styles.huyText}>Liên hệ với người mua</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.buttonContainer}
                onPress={() => handleAccept()}
              >
                <Text style={styles.logintext}>Nhận đơn</Text>
              </TouchableOpacity>
            </View>
          );
        case "preparing":
          return (
            <View style={[styles.namngang, { justifyContent: "flex-end" }]}>
              ...
              >
                <Text style={styles.logintext}>Đã vận chuyển</Text>
              </TouchableOpacity>
            </View>
          );
        case "delivering":
          return (
            <View style={[styles.namngang, { justifyContent: "flex-end" }]}>
              <Text
                style={[styles.name, { flex: 1, fontSize: 11, marginRight: 5 }]}
              >
                Đang đợi người mua cập nhật nhận hàng thành công
              </Text>
              <TouchableOpacity
                style={[
                  styles.buttonContainer2,
        ...
        case "sold":
          return (
            <View style={[styles.namngang, { justifyContent: "flex-end" }]}>
              <Text style={[styles.name, { fontSize: 11, marginRight: 5 }]}>
                Ngày hoàn thành: {formattedDate(order.completeDate)}
              </Text>
            </View>
          );
        default:
        ...
        <View style={{ paddingVertical: scaleHeight(10) }}>
          ...
          {isExpanded &&
            order.items
              .slice(1)
              .map((item, index) => (
                <View key={index}>{itemDetail(item)}</View>
              ))}
        </View>
